...
YAML과 JSON의 매핑 키는 모든 깊이에서 고유해야 합니다. 중복 키가 있으면 어느 한 값을 임의로 선택하지 않고 설정 오류로 처리합니다. `portability.scan`에 쓰는 glob 패턴도 상대 경로여야 하며 `..` 상위 이동을 포함할 수 없습니다.
...
검사 대상과 일치한 각 항목은 읽기 전에 실제 위치를 확인합니다. 외부 또는 끊어진 심볼릭 링크는 따라가지 않고 오류로 보고합니다.
